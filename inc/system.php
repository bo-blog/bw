...
class bwArticle
{
	...
	public function addArticle ($smt)
	{
		...
		bw::$db->dbExec ('INSERT INTO articles (aID, aTitle, aCateURLName, aTime, aTags, aReads, aContent) VALUES (?, ?, ?, ?, null, 0, ?)', array ($smt['aID'], $smt['aTitle'], $smt['aCateURLName'], $smt['aTime'], $smt['aContent']));
	...
	{ 
		...
		bw::$db->dbExec ('UPDATE articles SET aTitle=?, aCateURLName=?, aTime=?, aContent=? WHERE aID=?', array ($smt['aTitle'], $smt['aCateURLName'], $smt['aTime'], $smt['aContent'], $smt['aID']));
	...
	{
		$acceptedKeys=array ('aTitle', 'aID', 'aContent', 'aCateURLName', 'aTime');
...
class bwView
{
	...
	public function haltWithSuccess ($successMsg)
